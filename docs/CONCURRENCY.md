...
@Lock(LockModeType.PESSIMISTIC_WRITE)
Optional<WaitlistEntry> findFirstByKlassIdAndStatusOrderByIdAsc(
        Long klassId, WaitlistStatus status);
```

**@Transactional 없으면 각 연산이 별개 트랜잭션으로 분리되어 원자성 보장 불가**
...
T=2  Thread B: 락 획득 → reservedCount(1) >= capacity(1) → waitlist_entry 생성(WAITING), 커밋 → 락 해제
T=3  Thread C: 락 획득 → reservedCount(1) >= capacity(1) → waitlist_entry 생성(WAITING), 커밋 → 락 해제
...
④ reservedCount >= capacity
     → waitlist_entry 생성(WAITING, id 기반 순서 자동 부여)
...
```
① findByIdWithLock(klassId)                   klass 락 획득
② [PENDING/CONFIRMED] enrollment 행 재조회 (PESSIMISTIC_WRITE) — confirm과의 동시 실행 직렬화
   [WAITLISTED] waitlist_entry 재조회 (일반 읽기 — klass 락으로 이미 직렬화됨)
③ 이미 CANCELLED이면 → 400 (중복 취소 방지)
...
⑥ [PENDING/CONFIRMED 취소 한정] WAITING 대기자 승격 처리:
     MIN(id)인 WAITING 대기자 1명 조회 (PESSIMISTIC_WRITE)
...
**대기 순번은 id 기준으로 실시간 계산. 중간 취소로 gap이 생겨도 WAITING 항목만 세므로 연속 순번 반환:**
```sql
SELECT COUNT(*) + 1
FROM waitlist_entry
WHERE klass_id = ? AND status = 'WAITING' AND id < ?;
```
API 응답의 `waitlistPosition`은 raw id 값이 아닌 COUNT 기반 계산 결과를 반환한다.

---

## 대기자 조회 쿼리

인덱스 `(klass_id, status, id)` 활용:
...
|------|---------|--------------------------|------|
| 수강 신청 | O | — | reservedCount 증가, waitlist_entry id 기반 순서 보장, 중복 신청 직렬화 |
| 결제 확정 | X | O (enrollment 행) | PENDING→CONFIRMED 상태 전이 보호 |
| 취소 (PENDING/CONFIRMED) | O | O (enrollment 행) | reservedCount 감소, 대기자 승격, confirm과의 동시 실행 직렬화 |
...
reserved_count를 증가시키며, 자리가 없으면 waitlist_entry를 WAITING 상태로 생성한다.
대기 순서는 별도 sequence 컬럼 없이 waitlist_entry.id (auto increment) 기준으로 결정된다.

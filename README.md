...
비관적 락(Pessimistic Write Lock)으로 동시 신청 경쟁을 처리하고, 정원 초과 시 대기열(Waitlist)을 운영합니다.
...
수강 신청과 취소는 `klass` 행에 `SELECT FOR UPDATE` 락을 걸어 요청을 직렬화합니다. 낙관적 락은 충돌 시 애플리케이션에서 재시도 로직을 직접 구현해야 하고, 경쟁이 높은 환경에서는 반복 실패로 오히려 성능이 저하될 수 있습니다. Redis 분산 락은 현재 단일 DB 환경에서는 오버엔지니어링입니다. DB 비관적 락은 여러 애플리케이션 서버가 동일한 DB primary를 사용하는 환경에서도 유효하므로 현재 구조에 가장 적합합니다.
...
본 과제는 Claude (Anthropic)와 OpenAI Codex를 활용하여 개발하였습니다.
...
class_registration  (집계 루트 — 모든 신청 이력)
  ├── enrollment     (자리 확보 신청 — PENDING / CONFIRMED)
...
| 클래스 | 종류 | 설명 |
|--------|------|------|
| `EnrollmentServiceTest` | 단위 테스트 (Mockito) | 수강 신청/확정/취소 비즈니스 로직 검증 |
| `EnrollmentConcurrencyTest` | 동시성 통합 테스트 (`@SpringBootTest`) | 동시 수강 신청 시 정원 초과 없음, 대기열 처리 검증 |
